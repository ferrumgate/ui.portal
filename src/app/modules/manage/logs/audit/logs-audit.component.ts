...
export class LogsAuditComponent implements OnInit, OnDestroy {
  ...
  displayedColumns: string[] = ['position', 'insertDate', 'username', 'message', 'messageSummary', 'messageDetail'];
  dataSource: AuditLog[] = [];
  pageSize = 10;
  page = 0;
  totalLogs = 0;
  ...
  search() {
    ...
    return this.auditService.get(this.startDate.toISOString(), this.endDate.toISOString(), this.searchKey, undefined, undefined).pipe(
      map(y => {
